...
export default {
   ...
   created() {
      ...
      this.$axios
         .get(
            "http://4j89034j54.zicp.vip:8988/elm/websocket/checkOrderMessage/" +
               this.user.userId
         )
         .then((response) => {
   ...
   methods: {
      ...
      deleteTest(mid) {
         this.$axios
            .get("http://4j89034j54.zicp.vip:8988/elm/message/eliminate/" + mid)
            .then((response) => {
               this.$router.go(0);
            })
            .catch((error) => {
      ...
      clearWarning(mide, index) {
         ...
         this.$axios
            .post(
               "http://4j89034j54.zicp.vip:8988/elm/message/updateOrderByIdState",
               {
                  mid: mide,
                  status: 1,
               }
      ...
      wsInit() {
         const wsuri =
            "ws://4j89034j54.zicp.vip:8988/elm/websocket/" + this.user.userId;
         this.ws = wsuri;
